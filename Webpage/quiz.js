...
(function () {
  ...
  function domainKey(q) {
    return String(q.domain).charAt(0);
  }
  ...
  function prepare(q) {
    ...
    return {
      ...
      type: q.type || "single",
      options: order.map(function (k) {
        return q.options[k];
      }),
      /* correct: array of shuffled position indices (0-based) */
      correct: q.correct.map(function (c) {
  ...
  function getDiff() {
    return diffSel ? diffSel.value : "all";
  }
  function currentPoolSize() {
    return poolFor(domainSel.value, getDiff()).length;
  }
  ...
  function quizSetup() {
    ...
    if (!n) {
      start.disabled = true;
      card.appendChild(el("p", "muted", "No questions match these filters."));
    }
  ...
  var NUMBER_WORDS = { 2: "two", 3: "three", 4: "four", 5: "five" };

  function renderQuestion() {
    var item = quiz.set[quiz.idx];
    var isMulti = item.type === "multi";
    var needCount = item.correct.length;
    quizArea.innerHTML = "";
    ...
    meta.appendChild(
      el(
        "span",
        "q-counter",
        "Question " + (quiz.idx + 1) + " / " + quiz.set.length,
      ),
    );
    ...
    meta.appendChild(
      el(
        "span",
        "diff-badge " + (DIFF_CLS[item.difficulty] || "diff-medium"),
    ...
    /* "Select N" banner */
    if (isMulti) {
      var needWord = NUMBER_WORDS[needCount] || String(needCount);
      card.appendChild(
        el("div", "q-select-two", "★ Select " + needWord + " answers"),
      );
    }
    ...
    item.options.forEach(function (opt, i) {
      ...
      if (isMulti) {
        b.addEventListener("click", function () {
          ...
          if (submitBtn) submitBtn.disabled = selected.length !== needCount;
        });
      } else {
        b.addEventListener("click", function () {
          choose(i, card);
  ...
  function appendNextBtn(card) {
    ...
    next.addEventListener("click", function () {
      quiz.idx++;
      if (quiz.idx < quiz.set.length) renderQuestion();
      else renderResults();
    });
  ...
  function renderResults() {
    ...
    var missed = quiz.answered.filter(function (a) {
      return !a.right;
    });
    if (missed.length) {
      ...
      missed.forEach(function (a) {
        var chosenText = a.chosen
          .map(function (i) {
            return a.item.options[i];
          })
          .join(", ");
        var correctText = a.item.correct
          .map(function (i) {
            return a.item.options[i];
          })
          .join(", ");
        ...
        r.innerHTML =
          ...
          esc(a.item.stem) +
          "</div>" +
          '<div class="ra wrong">Your answer: ' +
          esc(chosenText) +
          "</div>" +
          '<div class="ra right">Correct: ' +
          esc(correctText) +
          "</div>" +
          '<div class="re">' +
          esc(a.item.e) +
  ...
  function renderFlash() {
    ...
    var corrTexts = c.correct
      .map(function (letter) {
        return c.options[letter];
      })
      .join(" / ");
    $("flashBack").innerHTML =
      "<b>" +
  ...
  function pbqSetup() {
    ...
    card.appendChild(
      el(
        ...
        "Hands-on tasks like the real exam — match ports, sort protocols, categorize controls, and order processes. " +
          "A random set of <strong>" +
          Math.min(PBQ_PER_SET, pool.length) +
          "</strong> is drawn from " +
          "<strong>" +
          pool.length +
          "</strong> labs, graded per row.",
      ),
    );
    ...
    if (!pool.length) {
      card.appendChild(
        el("p", "muted", "No PBQs for this domain — choose another."),
      );
  ...
  function renderPbq() {
    ...
    var meta = el("div", "quiz-meta");
    meta.appendChild(
      el("span", "q-counter", "Lab " + (pbq.idx + 1) + " / " + pbq.set.length),
    );
    meta.appendChild(
      el("span", "q-domain", item.sub + " · Domain " + item.d + ".0"),
    );
    ...
    shuffle(item.rows).forEach(function (row) {
      ...
      optionsShuffled.forEach(function (o) {
        sel.appendChild(new Option(o, o));
      });
  ...
  function gradePbq(card, item) {
    ...
    rows.forEach(function (line) {
      ...
      if (sel.value === sel.dataset.answer) {
        ...
        line.classList.add("row-incorrect");
        mark.textContent = "✗";
      }
    });
  ...
  function renderPbqResults() {
    pbqArea.innerHTML = "";
    var solved = pbq.results.filter(function (r) {
      return r.allRight;
    }).length;
    var rowsRight = pbq.results.reduce(function (a, r) {
      return a + r.correctRows;
    }, 0);
    var rowsTotal = pbq.results.reduce(function (a, r) {
      return a + r.total;
    }, 0);
    ...
    card.appendChild(
      el(
        "h3",
        null,
        "You fully solved " + solved + " of " + pbq.set.length + " labs",
      ),
    );
    card.appendChild(
      el(
  ...
  function renderPbqAdvQuestion(lab) {
    ...
    meta.appendChild(
      el("span", "q-domain", lab.title + " · Domain " + lab.domain),
    );
    meta.appendChild(
      el(
        "span",
        "q-counter",
        "Q" + (pbqAdv.qIdx + 1) + " / " + lab.questions.length,
      ),
    );
    pbqArea.appendChild(meta);
    ...
    if (isMulti) {
      ...
      card.appendChild(
        el("div", "q-select-two", "★ Select " + howMany + " answers"),
      );
    }
    ...
    letters.forEach(function (letter) {
      ...
      if (isMulti) {
        ...
        b.addEventListener("click", function () {
          gradeAdvQuestion(letter, card, lab, q, letters);
        });
      }
      opts.appendChild(b);
    });
    ...
    if (isMulti) {
      ...
      submitBtn.disabled = true;
      submitBtn.addEventListener("click", function () {
        gradeAdvMulti(selected, card, lab, q, letters);
      });
